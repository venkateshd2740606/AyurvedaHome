package com.ayurvedahome.engine
...
object AyurvedaHomeEngine {

    fun createInitialGame(level: AyurvedaHomeLevel): AyurvedaHomeGame = AyurvedaHomeGame(
        level = level,
    ...
    fun pourCount(game: AyurvedaHomeGame, from: Int, to: Int): Int {
    ...
    fun canPour(game: AyurvedaHomeGame, from: Int, to: Int): Boolean = pourCount(game, from, to) > 0

    fun pour(game: AyurvedaHomeGame, from: Int, to: Int): AyurvedaHomeGame {
    ...
    fun onTubeSelected(game: AyurvedaHomeGame, tubeId: Int): AyurvedaHomeGame {
    ...
    fun isWon(tubes: List<List<Int>>, level: AyurvedaHomeLevel): Boolean =
        tubes.all { tube -> tube.isEmpty() || tube.distinct().size == 1 }

    fun isWon(game: AyurvedaHomeGame): Boolean = isWon(game.tubes, game.level)

    fun getHintMove(game: AyurvedaHomeGame): Pair<Int, Int>? {
    ...
    fun solve(game: AyurvedaHomeGame): List<Pair<Int, Int>>? {
        if (isWon(game)) return emptyList()

        data class Node(val state: AyurvedaHomeGame, val path: List<Pair<Int, Int>>)
    ...
    fun optimalMoveCount(game: AyurvedaHomeGame): Int =
