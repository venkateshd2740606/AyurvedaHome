...
import com.ayurvedahome.domain.model.PuzzleArchetype
import com.ayurvedahome.domain.model.PuzzleProfile
import com.ayurvedahome.domain.model.PuzzleProfileMetrics
import com.ayurvedahome.domain.model.AyurvedaHomeGame
import com.ayurvedahome.domain.model.SkillCategory
...
object PuzzleProfileEngine {
    ...
    fun updateMetrics(current: PuzzleProfileMetrics, game: AyurvedaHomeGame): PuzzleProfileMetrics {
        if (game.status != GameStatus.COMPLETED || game.level.isTutorial) return current

        val moveCount = max(game.moves, 1)
        val optimalMoves = AyurvedaHomeEngine.optimalMoveCount(game)
