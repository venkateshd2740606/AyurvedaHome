...
import com.ayurvedahome.domain.model.GenerationProfile
import kotlin.random.Random

object AyurvedaHomeGenerator {
    ...
    fun generate(
        ...
        repeat(64) {
            ...
            if (AyurvedaHomeEngine.validateLevel(level)) return level
            attemptSeed++
        }
    ...
    fun generateForChallenge(
        ...
        levelNumber: Int,
        difficulty: Difficulty
    ): AyurvedaHomeLevel = generate(seed, levelNumber, difficulty)
    ...
    private fun buildLevel(
        ...
        val capacity = AyurvedaHomeLevel.DEFAULT_CAPACITY
    ...
    private fun scrambleFromSolved(
        ...
        while (made < moves && attempts < moves * 20) {
            ...
            if (from == to) continue
            val game = AyurvedaHomeEngine.createInitialGame(
                AyurvedaHomeLevel(
                    seed = 0,
                    levelNumber = 1,
            ...
            if (!AyurvedaHomeEngine.canPour(game, from, to)) continue
            val updated = AyurvedaHomeEngine.pour(game, from, to)
